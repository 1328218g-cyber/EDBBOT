...
class SpoonClient {
      ...
      const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            // ⭐ 멤버십/구독자 힌트 필드 수집 (키 이름에 관련 단어 포함된 필드를 모두 수집)
            //    어떤 필드가 실제 유료 구독자 표시인지 아직 모르므로 '추정 가능한 모든 필드'를 보관
            const hintKeys = Object.keys(profile).filter(k => {
              const lk = k.toLowerCase();
              return /(member|vip|sub|grade|tier|fan|paid|premium|role|badge|level|item)/.test(lk)
                  && !/member_count|total_member_count/.test(lk);
            });
            const membershipHints = {};
            for (const k of hintKeys) {
              const v = profile[k];
              // 원시값이나 짧은 객체만 보관 (너무 큰 배열/객체 제외)
              if (v === null || typeof v === 'number' || typeof v === 'boolean' || typeof v === 'string') {
                membershipHints[k] = v;
              } else if (Array.isArray(v) && v.length <= 10) {
                membershipHints[k] = v;
              } else if (typeof v === 'object' && JSON.stringify(v).length < 500) {
                membershipHints[k] = v;
              }
            }

            resolve({ tag, nickname, imgUrl, membershipHints, _allKeys: Object.keys(profile) });
          } catch (e) { 
            resolve(null); 
          }
        });
  ...
  // ⭐ 진단용: 캐시 우회하여 프로필 JSON 전체를 그대로 반환
  //    멤버십/구독자 필드를 탐색하기 위해 사용 (모든 top-level 필드 유지)
  async fetchUserProfileRaw(liveId, userId, accessToken) {
    if (!userId || !liveId || !accessToken) return null;
    const rawToken = accessToken.replace('Bearer ', '');
    return new Promise((resolve) => {
      const options = {
        hostname: 'kr-api.spooncast.net',
